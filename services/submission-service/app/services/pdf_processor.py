...
class PDFProcessor:
    ...
    def _compile_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            'flow_cell': re.compile(r'Flow\s*Cell\s*Selection[:\s]*\n?\s*(MinION|PromethION)\s*Flow\s*Cell', re.IGNORECASE | re.MULTILINE),
            'genome_size': re.compile(r'Approx\.\s*Genome\s*Size\s+(\d+(?:\.\d+)?)', re.IGNORECASE),
            'coverage': re.compile(r'Approx\.\s*Coverage\s*Needed\s+(\d+x?-?\d*x?)', re.IGNORECASE),
            'cost': re.compile(r'Projected\s*Cost[:\s]*\$?([\d,]+\.?\d*)', re.IGNORECASE),
            'basecalling': re.compile(r'basecalled\s*using[:\s]*\n\s*(HAC|SUP)\s*\([^)]+\)', re.IGNORECASE | re.MULTILINE),
            ...
            'flow_cell_count': re.compile(r'Estimated\s*number\s*of\s*Flow\s*Cells\s+(\d+)', re.IGNORECASE),
            'billing_account': re.compile(r'Billing\s*Account[:\s]*([^\n]+)', re.IGNORECASE),
            'special_instructions': re.compile(r'(?:Additional\s*Comments?\s*/?\s*Special\s*Needs?|Special\s*Instructions?)[:\s]*\n?\s*([^$]+?)(?=\n\n|\nBioinformatics|$)', re.IGNORECASE | re.MULTILINE | re.DOTALL),
