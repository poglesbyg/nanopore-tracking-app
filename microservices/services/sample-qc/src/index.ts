...
import { SampleQCService } from './services/SampleQCService'
...
// Initialize sample QC service
const sampleQCService = new SampleQCService({
  database: databaseService,
  messageQueue: messageQueueService,
  redis: redisService,
  logger
...
// Routes
app.use('/api/v1', setupRoutes(sampleQCService, healthCheckService, metricsService))
...
const gracefulShutdown = async (signal: string) => {
  ...
  server.close(async () => {
    try {
      await sampleQCService.shutdown()
...
const startServer = async () => {
  try {
    ...
    // Start sample QC service
    await sampleQCService.start()
    ...
    server.listen(PORT, () => {
      logger.info(`Sample QC service started on port ${PORT}`)
